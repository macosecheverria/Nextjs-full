...
export async function generateStaticParams(){
  ...
  const static152Pokemons = data.results.map(pokemon => ({
    name: pokemon.name
  }))
  

    return static152Pokemons.map(({name}) => ({
        name: name
    }))
}
...
const generateMetadata = async ({params}: Props):Promise<Metadata> => {
    try {
        ...
        return {
            ...
            description:"Alguna description del pokemon"
        }
    }
}
...
const getPokemon = async(name: string): Promise<Pokemon> => {
    try {
        const pokemon = await fetch(`http://pokeapi.co/api/v2/pokemon/${name}`).then(res => res.json());
